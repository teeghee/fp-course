...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b
  (<*>) sf sa = State (\s -> ((eval sf s) (eval sa (exec sf s)), exec sa (exec sf s)))
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  (=<<) f sa = State (\s -> runState (f . fst $ (runState sa s)) (snd $ runState sa s))
...
findM ::
  ...
  -> List a
  -> f (Optional a)
--findM g = foldRight (\a -> lift3 ifThenElse (g a) (pure (Full a))) (pure Empty)
findM _ Nil = pure Empty
findM g (a :. as) = (\p -> if p then pure (Full a) else findM g as) =<< g a
